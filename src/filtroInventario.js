...
export default function FiltroInventario() {
  ...
  const [datosFltradosParaMostrar, setDatosFiltradosParaMostrar] = useState('');
  ...
  const filtradoMarca=(nombre) => {
    const respuesta = datosTablaInventario.filter((registro) 
  ...
  const filtradoEstadoDelEquipo =(estado) =>{
    const respuesta = datosTablaInventario.filter((registro)
    => registro.estadoDelEquipo === estado
    );  
    return respuesta;
  };
